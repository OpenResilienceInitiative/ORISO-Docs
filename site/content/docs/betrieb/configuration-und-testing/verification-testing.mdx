...
kubectl describe ingress -n caritas | grep -A 5 "app.oriso.org"
```
...
# Test token endpoint
curl -X POST "https://auth.oriso.org/realms/online-beratung/protocol/openid-connect/token" \
...
ACCESS_TOKEN=$(curl -s -X POST "https://auth.oriso.org/realms/online-beratung/protocol/openid-connect/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "client_id=app" \
  -d "username=testuser" \
  -d "password=testpass" \
...
curl -H "Authorization: Bearer $ACCESS_TOKEN" \
  https://api.oriso.org/api/tenants | jq .

curl -H "Authorization: Bearer $ACCESS_TOKEN" \
  https://api.oriso.org/api/users/data | jq .
```
...
- [ ] Frontend is accessible (`curl -I https://app.oriso.org`)
- [ ] Admin is accessible (`curl -I https://admin.oriso.org`)
- [ ] API is accessible (`curl -I https://api.oriso.org`)
- [ ] Keycloak is accessible (`curl -I https://auth.oriso.org`)
- [ ] Matrix is accessible (`curl -I https://matrix.oriso.org`)
- [ ] Health endpoints return UP (`curl https://api.oriso.org/actuator/health`)
...
- [Post-Deployment](/betrieb/configuration-und-testing/post-deployment) - Complete post-deployment steps
- [Troubleshooting](/betrieb/operations/troubleshooting) - Common issues and solutions
- [Quick Reference](/betrieb/operations/quick-reference) - Essential commands
