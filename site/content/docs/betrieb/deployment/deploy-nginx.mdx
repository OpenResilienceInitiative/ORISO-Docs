...
<Callout>
If not installed, see [Install Required Software](/betrieb/getting-started/install-software) and [Setup Kubernetes](/betrieb/getting-started/setup-kubernetes).
</Callout>
...
- `api.oriso.org` → `$SERVER_IP`
- `app.oriso.org` → `$SERVER_IP`
- `admin.oriso.org` → `$SERVER_IP`
- `auth.oriso.org` → `$SERVER_IP`
- `matrix.oriso.org` → `$SERVER_IP`
...
- [DNS & SSL Configuration](/betrieb/configuration-und-testing/dns-ssl) - Configure DNS records
- [Post-Deployment](/betrieb/configuration-und-testing/post-deployment) - Complete post-deployment steps
- [Verification Testing](/betrieb/configuration-und-testing/verification-testing) - Test all endpoints
