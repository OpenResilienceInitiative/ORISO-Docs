...
<Callout>
The May-2026 Figma upgrades the AI track from "planned" to **active design**: a dedicated **Chat Zusammenfassen** button (`⇧Ü`) appears in the room settings, paired with a **Mark Text + PII Blur** toolbar in the message context menu. This page replaces the previously planned-only [4.1 Transcripts](/produkt/core-features/transcription).
</Callout>
...
- **Receiver has older client without blur support** → fallback: render the body in plain text but show a banner *"This message contains content the counselor wished to blur — please disregard."* (Decision needed — see [Figma analysis §4.4](/produkt/design-und-quality/figma-analysis-2026-05#44-pii-blur--durability-of-marks).)

## 4.6.5 Per-User Visibility — *"Message Visible to..."*

Both Mark and Blur can be combined with the **Multi-Recipient Send** (see [Group Chats §4.5.4](/produkt/core-features/group-chats#454-multi-recipient-send-wähle-wer-diese-nachricht-sehen-soll)) so that a marked, blurred excerpt is visible only to the moderators while the original message remains visible to all.

## 4.6.6 Permissions

These tools are **counselor-only** by default and can be globally toggled by admins. The Permissions Matrix (see [Roles & Permissions](/produkt/architecture-und-roles/roles-permissions#37-per-chat-type-permissions-matrix)) controls them per chat-type.
...
- [Group Chats (4.5)](/produkt/core-features/group-chats) — multi-recipient send pairs with these tools.
- [Roles & Permissions](/produkt/architecture-und-roles/roles-permissions) — per-chat-type matrix.
- [Edge Cases](/produkt/design-und-quality/edge-cases) — failure modes for AI, blur, marks.
- [Figma Analysis](/produkt/design-und-quality/figma-analysis-2026-05) — source screens.
