...
<Callout>
This page is the **single source of truth** for what changed in the product after the May-2026 Figma drop. It feeds the new pages in [Core Features](https://github.com/OpenResilienceInitiative/ORISO-Docs/blob/dev/product/features) and the targeted updates to [Roles & Permissions](/produkt/architecture-und-roles/roles-permissions), [Data Model](/produkt/flows-und-internals/data-model), [UI/UX](/produkt/design-und-quality/ui-ux), and [Edge Cases](/produkt/design-und-quality/edge-cases).
</Callout>
...
| **Open product question** | *"What happens when a user deletes himself during a video call (if he's the initiator)? What happens if a client deletes himself from a group chat?"* — see [Edge Cases](/produkt/design-und-quality/edge-cases#917-client-deletes-self-during-active-video-call) for the resolution we propose. |
...
| **Status** | **[NEW FEATURE]** — confirms the previously-planned AI tools track ([4.1 Transcripts](/produkt/core-features/transcription) → now [AI Tools](/produkt/core-features/ai-tools)). |
...
| F4 | **AI Chat Summarisation** | 1.10 | **[NEW FEATURE]** (replaces planned-only status of [4.1](/produkt/core-features/transcription)) |
...
- **Proposed**: see [Edge Cases §9.1.7](/produkt/design-und-quality/edge-cases#917-client-deletes-self-during-active-video-call). When a client deletes themselves mid-call, the LiveKit room continues for remaining counselors. The originator role transfers to the next counselor by `joined_at` ASC; if no counselor remains, the room ends.
