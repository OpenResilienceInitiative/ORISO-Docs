...
<Callout>
This page is the developer's deep-dive. Cross-references back to the architecture page ([Section 2](/produkt/architecture-und-roles/architecture)) and the data model ([Section 7](/produkt/flows-und-internals/data-model)) are heavy on purpose — they are the same system seen from different angles.
</Callout>
