# ADR-003: Department = unique (Agency × Topic) carrying its own imprint + DPP (/decisions/adr-003)



> **⚠️ Partially superseded by [ADR-014](/decisions/adr-014) (2026-07-16).** Decision #3 (admin single-select
...
> (`dpp_id`/`imprint_id`). The UNIQUE(agency\_id, topic\_id) constraint and per-department
> imprint/DPP from this ADR remain in force. Read [ADR-014](/decisions/adr-014) before acting on this document.
>
...
* **Related:** [ADR-001](/decisions/adr-001) (modalities as modules — topic/agency/modality orthogonal);
  [ADR-002](/decisions/adr-002) (silent membership); `ORISO-UserService/CONTEXT.md`; AgencyService `Agency`,
...
The silent-membership routing ([ADR-002](/decisions/adr-002)) needs a clean "topic → the one department → its
counsellors + its imprint/DPP".

## Decision [#decision]

1. **Department is the first-class unit** = a **unique** (agency\_id, topic\_id). Enforce
...
## Implementation status (2026-07-01) [#implementation-status-2026-07-01]
...
## Implementation status (2026-07-03) [#implementation-status-2026-07-03]
...
## Implementation status (2026-07-07) [#implementation-status-2026-07-07]
...
broader case-handover client view (#204), not [ADR-003](/decisions/adr-003) proper.
