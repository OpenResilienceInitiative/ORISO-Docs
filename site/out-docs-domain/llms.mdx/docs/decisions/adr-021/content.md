# ADR-021: Legal-text hierarchy, generic versioning, and the consent text as a field of the DPP (/decisions/adr-021)
...
[ADR-003](/decisions/adr-003) and [ADR-014](/decisions/adr-014) established that a Department (Agency × Topic) carries its own imprint and
...
### Measured current state (2026-08-16, `origin/pre-dev` unless noted) [#measured-current-state-2026-08-16-originpre-dev-unless-noted]
...
7. **The imprint is an information duty and never a consent gate.** It requires guaranteed
   ...
   Only the DPP (and, between platform and Träger, the documents in [ADR-023](/decisions/adr-023)) are consent-bearing.
...
both the policy and its consent wording (see [ADR-022](/decisions/adr-022)); the existing editor UI is reused as-is.
