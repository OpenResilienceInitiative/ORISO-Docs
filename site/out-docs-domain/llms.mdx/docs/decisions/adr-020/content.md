# ADR-020: Scheduled calls, secure invitations, and a unified contact calendar (/decisions/adr-020)
...
* **Related:** [ADR-006](/decisions/adr-006) (persisted conversation modality), [ADR-012](/decisions/adr-012) (group occurrences and Future Timeline), [ADR-018](/decisions/adr-018) in ORISO-Frontend (Element Call Matryoshka integration), ORISO-Frontend#974
...
### 1. Separate Appointment, Call Session, and Call Invitation [#1-separate-appointment-call-session-and-call-invitation]
...
### 2. Make call access a tenant policy enforced by the backend [#2-make-call-access-a-tenant-policy-enforced-by-the-backend]
...
### 3. Use one entry composition for QR, copied links, registered users, and guests [#3-use-one-entry-composition-for-qr-copied-links-registered-users-and-guests]
...
### 4. Provide one contact calendar across modalities [#4-provide-one-contact-calendar-across-modalities]
...
### 5. Model availability blockers and conflict deadlines explicitly [#5-model-availability-blockers-and-conflict-deadlines-explicitly]
...
### 6. Turn the conversation projection into a real Future Timeline [#6-turn-the-conversation-projection-into-a-real-future-timeline]
...
Appointments and group occurrences use one typed future-event card/session primitive. Existing bounded loading from [ADR-012](/decisions/adr-012) remains: the client must not materialize an unbounded future. Appointment-only users must see the projection even when no group series exists.

### 7. Distinguish persistent entry links from live-call invitations [#7-distinguish-persistent-entry-links-from-live-call-invitations]
...
### 8. Use versioned call-timing presets [#8-use-versioned-call-timing-presets]
...
### 9. Preserve confidentiality in external calendar data [#9-preserve-confidentiality-in-external-calendar-data]
...
## Bounded-context ownership [#bounded-context-ownership]
...
## Required implementation slices [#required-implementation-slices]
