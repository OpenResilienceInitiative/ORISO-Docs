# ADR-014: Shared legal-text objects, multi-topic agencies, and topic-before-consent (/decisions/adr-014)
...
> (2026-07-18) and became &#x2A;*[ADR-019](/decisions/adr-019)**. This record keeps 014, so an unqualified "[ADR-014](/decisions/adr-014)" now
> means shared legal text objects and topic-before-consent.

> **Extended 2026-08-16 by `ADR-021`.** Nothing here is reversed. [ADR-021](/decisions/adr-021) adds what this record
...
[ADR-003](/decisions/adr-003) correctly made the Department = unique (agency\_id, topic\_id) the carrier of the
...
1. **The single-select admin topic picker** ([ADR-003](/decisions/adr-003) decision #3, shipped as ORISO-Admin PR #244)
...
1. **Agencies are multi-topic again.** The admin topic picker returns to **multi-select**
   (reverses [ADR-003](/decisions/adr-003) decision #3 / ORISO-Admin PR #244). Existing multi-topic agencies are
   **valid data**, not dirt to clean up. Department = unique (agency × topic), the `UNIQUE`
   constraint, and the draft|published lifecycle from [ADR-003](/decisions/adr-003) all stand unchanged.
2. **Legal texts become first-class shared objects.** New `legal_text` entity
   (id, tenant\_id, kind = DPP | IMPRINT, label, multilingual content, publication\_status).
