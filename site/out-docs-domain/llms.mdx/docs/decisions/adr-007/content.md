# ADR-007: Live-chat liveness — the indicator reads backend Availability, never mirrors or infers it (/decisions/adr-007)
...
## Considered options (badge gating) [#considered-options-badge-gating]

* **Gate the per-session badge on real-time RC/Matrix presence.** Most "honest", but couples the fix to the unreliable presence layer and the [ADR-005](/decisions/adr-005) Matrix rebuild. &#x2A;*Rejected for now.**
...
## Resolved product questions (Frank, 2026-06-28) [#resolved-product-questions-frank-2026-06-28]
...
3. **Badge gating** → &#x2A;*option (b)**: gate the per-session "live" badge on Availability / active-session state, not real RC/Matrix presence — keeps the whole fix transport-agnostic and independent of [ADR-005](/decisions/adr-005).
...
## Status & progress (2026-06-30) [#status--progress-2026-06-30]
