# ADR-005: Matrix federation deliberately OFF; real DNS server_name via a clean homeserver rebuild (/decisions/adr-005)
...
* **Related:** \[\[1 Analysis/ADRS/[ADR-004](/decisions/adr-004)-chat-keep-custom-ui-adopt-matrix-sdk-megolm]] (SDK-Megolm adoption depends on stable MXIDs), findings K8-M09 / K8-M06 (insecure key handling) / DB-H04 / DB-M09 (bare-IP server\_name baked into MXIDs)

## PreDev implementation exception — 2026-07-11 [#predev-implementation-exception--2026-07-11]
...
* **MXID stability is a hard prerequisite** for adopting matrix-js-sdk Megolm crypto (\[\[1 Analysis/ADRS/[ADR-004](/decisions/adr-004)-chat-keep-custom-ui-adopt-matrix-sdk-megolm]]): device keys and key backup bind to MXIDs, so the `server_name` must be final before crypto is enabled.
...
**Positive:** stable, DNS-based MXIDs that survive IP changes; a smaller attack surface appropriate for a counselling tool; unblocks the SDK-Megolm crypto adoption in [ADR-004](/decisions/adr-004); resolves DB-H04 / K8-M09 / K8-M06 in one move.

**Negative / cost:** requires a clean rebuild and ops bandwidth (neusta); all current test accounts/rooms are discarded; it must happen **before** crypto adoption or that work is redone. The dependency makes [ADR-004](/decisions/adr-004) step 2 (SDK Megolm) wait on this.

## Status & progress (updated 2026-07-02) [#status--progress-updated-2026-07-02]
...
* The June-30 frontend work (\[\[1 Analysis/ADRS/[ADR-004](/decisions/adr-004)-chat-keep-custom-ui-adopt-matrix-sdk-megolm]])
