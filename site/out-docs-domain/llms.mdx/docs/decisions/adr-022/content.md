# ADR-022: Two consent gates, consent state as a session pointer, and re-consent on change (/decisions/adr-022)
...
* **Scope:** sessions that have a **help-seeker** — 1:1 counselling, live chat, self-help groups.
  Internal rooms (supervision per [ADR-008](/decisions/adr-008), Team-Besprechung per [ADR-016](/decisions/adr-016)) have no help-seeker and
...
### Measured current state (2026-08-16, `origin/pre-dev`) [#measured-current-state-2026-08-16-originpre-dev]
...
1. **Exactly two gates, no third.**
   * **Gate 1, entering the waiting room:** the platform-level policy (or the Träger's replacement,
     when a Träger is already resolved — see [ADR-021](/decisions/adr-021) decision 2). One document, one agreement.
...
2. **Consent state is a pointer on the session, not a consent log.** `session` gains a nullable
   ...
   **overwritten** on re-consent; no append-only history, no per-user audit trail, no behavioural
   ...
   personal data. Because [ADR-021](/decisions/adr-021) decision 4 attaches the consent wording to the DPP, this single
   pointer covers both.

3. **Proof of what applied is carried by the publication history, not by a user log.** "Which
   wording was in force from when to when" is answered by the version history from [ADR-021](/decisions/adr-021) decision
