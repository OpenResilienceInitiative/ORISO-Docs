...
The final independent Astra replay found 16 outdated basename-only ADR references in the historical narrative. They now use uniquely verified repository-relative document IDs. Actual narrative replay retains all intended relations, 11 tour steps and all four reviewed flows, with zero dropped references or missing statistics. The producer now rejects a malformed narrative report or nonempty dropped-reference/missing-stat arrays before sealing; 13 fault variants preserve the preceding complete generation. These changes preserve historical claim status and do not grant ADR authority.
