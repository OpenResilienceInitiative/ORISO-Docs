...
|-----------|-----------|---------|
| `api.example.org` | `$SERVER_IP` | Backend API |
| `app.example.org` | `$SERVER_IP` | Frontend application |
| `admin.example.org` | `$SERVER_IP` | Admin panel |
| `auth.example.org` | `$SERVER_IP` | Keycloak authentication |
| `matrix.example.org` | `$SERVER_IP` | Matrix Synapse |
| `element.example.org` | `$SERVER_IP` | Element.io client (optional) |
| `status.example.org` | `$SERVER_IP` | Status page (optional) |

<Note>
`example.org` is a placeholder. Replace it with your own domain (Helm `global.domainName`). DNS propagation may take a few minutes to hours.
</Note>
...
curl http://api.example.org/.well-known/acme-challenge/test
```
