...
Communication:
  ...
  Element Homeserver (login):
  - Local:      http://$SERVER_IP:8008
  - Via domain: https://chat.example.org (requires Nginx well-known)
...
<Step title="Validate Element homeserver discovery">

If you configured Nginx well-known endpoints, verify client discovery:
...
<Check>
The response should include `m.homeserver.base_url` pointing to your homeserver. Element should pre-fill the homeserver when you type your domain on the login screen.
</Check>
