...
- **Related:** `ORISO-Frontend/CONTEXT.md` (Activity Timeline glossary), `oriso-platform/dsfa-analysis/dsfa-deepdive-notification-center.md` (DSFA deep-dive 3, 2026-08-14: `event_notification` schema, `PATCH read-all`, preview mode fixed to NONE, `read_date` as activity profile), ADR-012 (Future Timeline / Activity Timeline sequencing), ADR-018 (one Activity Timeline entry per Erstantwort; exception to the system-notification filter), ADR-004/ADR-005 (Matrix + Megolm), the referenced-but-unpublished ADR-AT-01 "Storage & E2EE Boundary" (OrisoPlan WP-06), ORISO-Frontend issues #1200, #847, #845
...
1. **Bulk read is an activity-event operation only.** "Mark all as read" sets `read_date` on all of the user's `event_notification` rows via UserService `PATCH /users/event-notifications/read-all` (self-scoped; DSFA deep-dive §1.4). It MUST NOT touch Matrix read receipts, chat unread badges or session state; those belong to the Matrix room timeline, which is a different thing (`CONTEXT.md` "Timeline collision"). The affordance stays visible and is enabled only while the **server-side unread total** (`unreadCount` returned with the feed, mirrored live by the client) is greater than zero; otherwise it is disabled rather than a silent no-op. It MUST NOT be derived from the loaded page alone, because the feed is paginated and older pages may still hold unread events (#1200, review finding). Label stays "Mark all as read"; if the overload of "notification" causes confusion, the fix is copy ("Mark all activity as read"), decided by product, not a behaviour change.
...
- The DSFA deep-dive recommends reducing `read_date` to a read flag plus a retention job (§3c); this ADR does not depend on either — bulk read stays a single self-scoped write whatever the column shape.
