...
- **Start of the period:** `archive_date`. A discussion that was never archived and is still `OPEN` is measured from `create_date` under the same period, so an abandoned discussion cannot outlive an archived one. There is deliberately no separate "abandoned" state: the enquiry an `OPEN` discussion belongs to is by definition still unaccepted, and the product decision (#1116) is that a coordination room may not exist longer than the retention period in any state. Consequence, accepted: an enquiry that stays unaccepted beyond the period loses its Team-Besprechung room while the enquiry itself stays open; the team can open a new one. Should this prove too aggressive in practice, the fix is a longer period for the `OPEN` case, not a new state.
...
**Effective period, for the DPIA.** Proposed: **90 days after archiving** (or after creation, for a discussion never archived). There is no statutory figure for team coordination rooms; the Caritas professional position is that process data goes once the client relationship has ended. 90 days is the DPIA proposal and remains **pending the data protection officer's sign-off**; until then the deployed value is `0` (job off). It is deliberately a deployment value so the number can be set, and later corrected, without a code change.
