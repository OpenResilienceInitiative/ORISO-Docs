# ADR-017: Rebuild chat threads on native Matrix `m.thread` relations — hard cut, no dual-read

- **Status:** Accepted — Frank, 2026-07-18 (grill-with-docs session)
- **Date:** 2026-07-18
- **Deciders:** Frank (product) + AI (engineering)
- **Related:** ADR-004 (keep own UI, adopt Matrix SDK), ADR-016 (Team-Besprechung is flat — does not depend on this), memory `feedback-no-prod-users-skip-migration`, memory `oriso-team-besprechung-design`
