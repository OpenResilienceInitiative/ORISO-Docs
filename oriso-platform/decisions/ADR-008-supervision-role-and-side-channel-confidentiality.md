...
## Addendum 2026-07-13 (grill-with-docs): standing auto-assignment + unified opt-out consent
...
## Addendum 2026-09-05: standing supervision is always on — inform, do not offer an opt-out

**Decision (Frank, product, 2026-09-05).** For **standing supervision** ("Supervision (auto-assigned)", decisions 1–3 of the 2026-07-13 addendum), **point 4 of that addendum — the default-on, client-facing opt-out — is withdrawn.** Standing supervision is **always on and cannot be switched off by the advice seeker.** There is no opt-out switch in the chat and no exit clause.

**Rationale.** Standing supervision is not an extra disclosure of the counselling content to a new circle of recipients; it is part of how the counselling centre works. The supervisor is a trained professional **of the same counselling centre**, reads **read-only**, never writes into the client room, and is **invisible to the advice seeker**. A per-session switch would make the product more complicated than the thing it models, and it would let a client silently disable a quality-assurance measure the counselling centre is professionally responsible for.

**Instead: active disclosure in the chat.** The advice seeker is informed **by a system message in her own room**, at the latest at the moment a counsellor has accepted her enquiry — which is exactly the moment the standing supervisor is attached (`AssignEnquiryFacade` → `SessionSupervisorFacade.attachStandingSupervisorIfAssigned`). The message names only the **counselling centre**, never the supervisor, and points at the counselling centre's privacy notice. Only counselling centres with a standing supervisor configured are affected; a session without one gets no message.

**Consequences.**

- The existing endpoint `POST /users/sessions/{id}/supervision/opt-out` is **not offered in the UI**. The backend endpoint, `Session.supervisionOptedOut` and its Liquibase column stay in place for now; **removing them is an open follow-up**, tracked in the supervision epic, not decided here.
- The **case-handover** opt-out (US#313) is untouched — it remains default-on and client-facing. Only the supervision reasons move off that model.
- Implementation ticket: **ORISO-Frontend#1315** — "Advice seeker is told in the chat that a supervisor reads along (standing supervision)" (UserService posts the message, Frontend renders and translates it), sub-issue of the supervision epic ORISO-Frontend#1299.
- The exact wording remains, as in the original ADR, **subject to the data protection officer's approval**.

**Open for the DPO.** Informing at the moment of acceptance happens **after** the advice seeker has already written her enquiry — she has disclosed before she was told. The first-line information must therefore continue to sit **in the counselling centre's privacy notice at chat entry**, as the original ADR-008 body requires ("disclose via the agency's privacy policy + Impressum on chat entry"). Whether an entry-point privacy notice plus an in-chat system message at acceptance satisfies the information duties of **§ 15 KDG / Art. 13 GDPR** for standing supervision — and whether the timing is early enough — is a question for the data protection officer, not one this ADR settles.
