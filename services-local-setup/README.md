...
```bash
ORISO_DEV_KEYCLOAK_URL=https://auth.example.org \
  ./ORISO-Docs/services-local-setup/run-oriso-local.sh start --hybrid --ui admin
```

This starts MariaDB, MongoDB, Redis, RabbitMQ, all four backend services, the local API gateway (`localhost:8088`), and Admin UI. Auth uses the Keycloak in `ORISO_DEV_KEYCLOAK_URL`, which is required in hybrid mode (no default), e.g. `ORISO_DEV_KEYCLOAK_URL=https://auth.example.org`.
...
```bash
ORISO_MODE=hybrid ORISO_DEV_KEYCLOAK_URL=https://auth.example.org \
